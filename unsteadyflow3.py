...
#   Function to gather inputs, excluding initial conditions        
    def inputs ( self, u, p, x ):
        ...
        if ( self.len_p == self.len_u and self.len_p == self.len_x ):
            self.n = self.len_p
...
#       Start tcount at 1 so that it copies into time+1               
        tcount = 1
...
#               Locate Point 1
#               For the initial guess, use u[i], p[i]                                      
#               Use iterator function to calculate new values for C+
                [x1, u_pl, p_pl] = self.cp_iter( self.u[i], self.p[i], self.x[i], i )
            
#               Calculate Q+ and T+ from C+ 
                q_pl = self.calc_q( p_pl )
                t_pl = self.calc_t( p_pl, u_pl, 1 )              
                            
#               Locate Point 2
#               For the initial guess, use u[i+2], p[i+2]                       
#               Use iterator function to calculate new values for C-
                [x2, u_mn, p_mn] = self.cm_iter( self.u[i+2], self.p[i+2], self.x[i], i ) 
           
#               Calculate Q- and T-
                q_mn = self.calc_q( p_mn )
                t_mn = self.calc_t( p_mn, u_mn, 2 )
            
#               Determine values at next time interval                        
                a = array( [ [1.0, q_pl], [ 1.0, -q_mn  ]] )
                b = array( [ [t_pl], [t_mn] ] )
                [ p_fin, u_fin ] = solve( a, b )

#               Update arrays          
#               Update to (i+1)       
                self.p_arr[tcount][i+1] = p_fin
                self.u_arr[tcount][i+1] = u_fin
...
#           Calculate speed of sound            
            ...
            lmda = 1.0/( a + u )
            lmda *= 1000
            x1 = self.x[count+1] - (self.dt/lmda)
